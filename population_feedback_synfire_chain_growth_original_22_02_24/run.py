...
def run_test(m, output_dir_name, n_show_only=None, add_noise=True, dropouts={'E': 0, 'I': 0},
    w_r_e=None, w_r_i=None, epochs=500):
    ...
    for i_e in range(epochs):
        ...
        if i_e == 0:
            sio.savemat(robustness_output_dir + '/' + f'title_{title}_eidx_{zero_pad(i_e, 4)}', {
            ...
            e_total_potentiation = m.ETA * (m.ALPHA * fr_update_e + m.BETA * stdp_burst_pair + fr_pop_step)
...
def quick_plot(m, run_title='', w_r_e=None, w_r_i=None, n_show_only=None, add_noise=True, dropouts={'E': 0, 'I': 0}):
