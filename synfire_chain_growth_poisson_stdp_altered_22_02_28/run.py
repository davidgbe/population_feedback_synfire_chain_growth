...
M = Generic(
    ...
    W_E_E_R_MIN=1e-6,
    W_E_E_R_MAX=0.26 * 0.004 * 1.3 * 1.5,
...
M.KERNEL_PAIR_EE[:(M.CUT_IDX_TAU_PAIR_EE + int(4.4e-3 / S.DT) + 1)] *= -1
...
def run_test(m, output_dir_name, n_show_only=None, add_noise=True, dropouts={'E': 0, 'I': 0},
    ...
    w_u_uva = np.diag(np.ones(m.N_EXC - m.N_DRIVING_CELLS)) * m.W_U_E * 0.5

    w_u_e = np.zeros([m.N_EXC, m.N_DRIVING_CELLS + m.N_EXC])
    w_u_e[:m.N_DRIVING_CELLS, :m.N_DRIVING_CELLS] += w_u_proj
    w_u_e[m.N_DRIVING_CELLS:m.N_EXC, -(m.N_EXC - m.N_DRIVING_CELLS):] += w_u_uva
    ...
    w_u = {
        ...
        'E': np.block([
            [ w_u_e ],
            [ np.zeros([m.N_SILENT + m.N_INH, m.N_DRIVING_CELLS + m.N_EXC]) ],
        ]),

        'I': np.zeros((m.N_EXC + m.N_SILENT + m.N_INH , m.N_DRIVING_CELLS + m.N_EXC)),

        'A': np.zeros((m.N_EXC + m.N_SILENT + m.N_INH, m.N_DRIVING_CELLS + m.N_EXC)),
    }
    ...
    pairwise_spk_delays = np.block([
        [int(4.4e-3 / S.DT) * np.ones((m.N_EXC, m.N_EXC)), np.ones((m.N_EXC, m.N_INH))],
        [np.ones((m.N_INH, m.N_EXC + m.N_INH))],
    ]).astype(int)
    ...
    for i_e in range(epochs):
        ...
        ## inp spks
        spks_u_base = np.zeros((len(t), m.N_DRIVING_CELLS + m.N_EXC), dtype=int)
        ...
        def make_poisson_input(dur=0.2, offset=0.05):
            x = np.zeros(len(t))
            x[int(offset/S.DT):int(offset/S.DT) + int(dur/S.DT)] = np.random.poisson(lam=15 * S.DT, size=int(dur/S.DT))
            return x

        # uva_spks_base = np.random.poisson(lam=20 * S.DT, size=len(t))
        spks_u[:, m.N_DRIVING_CELLS:(m.N_EXC + m.N_DRIVING_CELLS)] = np.stack([make_poisson_input() for i in range(m.N_EXC)]).T
        ...
        spks_received_for_i_cells = rsp.spks_received[:, (m.N_EXC + m.N_SILENT):(m.N_EXC + m.N_SILENT + m.N_INH), (m.N_EXC + m.N_SILENT):(m.N_EXC + m.N_SILENT + m.N_INH)]
        ...
        if i_e > 0:
            ...
            stdp_burst_pair_e_e_plus = np.zeros([spks_for_e_cells.shape[1], spks_for_e_cells.shape[1]])
            stdp_burst_pair_e_e_minus = np.zeros([spks_for_e_cells.shape[1], spks_for_e_cells.shape[1]])
            ...
            for i_t in range(spks_for_e_cells.shape[0]):
                ...
                trimmed_kernel_ee_plus = m.KERNEL_PAIR_EE[M.CUT_IDX_TAU_PAIR_EE:M.CUT_IDX_TAU_PAIR_EE + (stdp_end_ee - i_t)]
                # print(trimmed_kernel_ee_plus)
                trimmed_kernel_ee_minus = m.KERNEL_PAIR_EE[M.CUT_IDX_TAU_PAIR_EE - (i_t - stdp_start_ee):M.CUT_IDX_TAU_PAIR_EE]
                # print(trimmed_kernel_ee_minus)

                for curr_spk_e in curr_spks_e.nonzero()[0]:
                    sparse_spks_received_e_plus = csc_matrix(filtered_spks_received_for_e_cells[i_t:stdp_end_ee, curr_spk_e, :])
                    sparse_spks_received_e_minus = csc_matrix(filtered_spks_received_for_e_cells[stdp_start_ee:i_t, curr_spk_e, :])
            ...
            w_r_copy['E'][:m.N_EXC, :m.N_EXC] += (firing_rate_potentiation * w_r_copy['E'][:(m.N_EXC + m.N_SILENT), :(m.N_EXC + m.N_SILENT)])
            w_r_copy['E'][:m.N_EXC, :m.N_EXC] += (stdp_ee_potentiation * (m.W_E_E_R_MAX * ee_connectivity - w_r_copy['E'][:(m.N_EXC + m.N_SILENT), :(m.N_EXC + m.N_SILENT)]))
            w_r_copy['E'][:m.N_EXC, :m.N_EXC] -= (stdp_ee_depression * w_r_copy['E'][:(m.N_EXC + m.N_SILENT), :(m.N_EXC + m.N_SILENT)])


            w_r_copy['E'][:m.N_EXC, :m.N_EXC][(w_r_copy['E'][:m.N_EXC, :(m.N_EXC + m.N_SILENT)] < m.W_E_E_R_MIN) & ee_connectivity] = m.W_E_E_R_MIN
